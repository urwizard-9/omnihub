...
import { Role, EventLog, SecurityState, DocRecord, ConceptNode, RiskData, UserProfile } from '../types';
...
interface OmniHubContextType {
    ...
    riskHistory: RiskData[]; // Changed from { time: string, score: number }[]
    refreshRiskData: () => Promise<void>; // Added manual refresh
    resetSecurity: () => void;
...
export const OmniHubProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const refreshRiskData = useCallback(async () => {
        if (!token) return;
        try {
            const rData = await BackendAPI.fetchRiskHistory(token);
            setRiskHistory(rData);

            // Update current security state from the latest record
            if (rData.length > 0) {
                const latest = rData[0];
                setSecurityState(prev => ({
                    ...prev,
                    riskScore: latest.riskScore,
                    mode: (latest.defconMode || 'SAFE').toUpperCase() as any, // "alert" -> "ALERT"
                }));
            }
        } catch (e) {
            console.error("Risk history fetch failed", e);
        }
    }, [token]);
    ...
    const enableAutoSync = useCallback(async () => {
        ...
        try {
            ...
            addLog("Auto-Sync Enabled", "INFO", "SYSTEM");
            alert("✅ Real-time Auto-Sync Enabled!\n(Webhook registered for your account)");
        } catch (e: any) {
            console.error("Auto-sync enable failed", e);
    ...
    const value = useMemo<OmniHubContextType>(() => ({
        ...
        token,
        docs, concepts, logs, securityState, riskHistory, refreshRiskData,
        ...
        token, docs, concepts, logs, securityState, riskHistory, refreshRiskData,
