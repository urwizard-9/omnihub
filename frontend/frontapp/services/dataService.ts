...
import { ConceptNode, DocRecord, Role, SecurityLevel, DriveFile, RiskData, UserProfile } from '../types';
...
export const BackendAPI = {
    ...
    // 5. User Management (Profile & Admin)
    async fetchRiskHistory(token: string): Promise<RiskData[]> {
        try {
            const response = await fetch(`${API_BASE_URL}/api/risk/history?limit=100`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
    ...
    // [NEW] Fetch Specific User History (Real)
    fetchUserRiskHistory: async (userId: string, token: string): Promise<RiskData[]> => {
        try {
            const response = await fetch(`${API_BASE_URL}/api/risk/history/${userId}?limit=50`, {
                headers: {
                    'Authorization': `Bearer ${token}`
                }
            });
            if (!response.ok) throw new Error("Failed to fetch user risk history");
