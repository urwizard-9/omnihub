...
const GraphCanvas = memo(({
    ...
    // Helper: Deterministic Hash for Initial Positions
    const getDeterministicPos = (id: string) => {
        ...
        // Map hash to a coordinate execution space (e.g., -300 to 300)
        const range = 600;
        const x = ((Math.abs(hash) % range) - range / 2);
        const y = ((Math.abs(hash >> 16) % range) - range / 2);
        return { x, y };
    };

    // 1. Generate Raw Nodes/Links based on current ViewMode/Filters (Deterministic)
    const { rawNodes, rawLinks } = useMemo(() => {
        ...
        const addNode = (n: GraphNode) => {
            if (!addedNodeIds.has(n.id)) {
                // Apply deterministic initial position if not present
                if (n.x === undefined || n.y === undefined) {
                    const { x, y } = getDeterministicPos(n.id);
                    n.x = x;
                    n.y = y;
                }
                nodes.push(n);
        ...
        // Helper: Build Meaningful Edges
        const buildMeaningfulEdges = (subsetDocs: DocRecord[]) => {
            const docMap = new Map<string, DocRecord>();
            const conceptDocsMap = new Map<string, Set<string>>(); // ConceptID -> Set<DocID>

            // 1. Doc-Concept Edges & Build Maps
            subsetDocs.forEach(d => {
                docMap.set(d.id, d);

                // Existing Concepts
                d.conceptIds.forEach(cid => {
                    const c = concepts.find(xx => xx.id === cid);
                    if (c) {
                        links.push({ source: cid, target: d.id, type: 'evidence' }); // Concept -> Doc
                        if (!conceptDocsMap.has(cid)) conceptDocsMap.set(cid, new Set());
                        conceptDocsMap.get(cid)?.add(d.id);
                    }
                });

                // Tags as Virtual Concepts
                d.tags.forEach(tag => {
                    const existingC = concepts.find(c => c.label === tag);
                    if (existingC) {
                        links.push({ source: existingC.id, target: d.id, type: 'evidence' });
                        if (!conceptDocsMap.has(existingC.id)) conceptDocsMap.set(existingC.id, new Set());
                        conceptDocsMap.get(existingC.id)?.add(d.id);
                    } else {
                        const vId = `tag:${tag}`;
                        addNode({ id: vId, label: tag, type: 'concept', data: { id: vId, label: tag } });
                        links.push({ source: vId, target: d.id, type: 'evidence' });

                        // Treat tag as concept for co-occurrence? Maybe skip for noise reduction
                    }
                });
            });

            // 2. Concept -> Concept (Co-occurrence)
            const conceptIds = Array.from(conceptDocsMap.keys());
            for (let i = 0; i < conceptIds.length; i++) {
                for (let j = i + 1; j < conceptIds.length; j++) {
                    const c1 = conceptIds[i];
                    const c2 = conceptIds[j];
                    const docs1 = conceptDocsMap.get(c1)!;
                    const docs2 = conceptDocsMap.get(c2)!;

                    // Intersection
                    let intersection = 0;
                    docs1.forEach(did => { if (docs2.has(did)) intersection++; });

                    if (intersection >= 2) { // Threshold: appeared together in at least 2 docs
                        links.push({ source: c1, target: c2, type: 'related' });
                    }
                }
            }

            // 3. Doc -> Doc (Shared Concepts)
            // Can be expensive O(N^2), limit to small subset or use high threshold
            if (subsetDocs.length < 100) {
                for (let i = 0; i < subsetDocs.length; i++) {
                    for (let j = i + 1; j < subsetDocs.length; j++) {
                        const d1 = subsetDocs[i];
                        const d2 = subsetDocs[j];

                        // Count shared concepts
                        const shared = d1.conceptIds.filter(c => d2.conceptIds.includes(c)).length;
                        if (shared >= 2) { // Threshold: share at least 2 concepts
                            links.push({ source: d1.id, target: d2.id, type: 'related' });
                        }
                    }
                }
            }
        ...
        if (effectiveMode === 'evidence' && evidenceData) {
            // EVIDENCE MODE: Show purely what is in evidenceData
            evidenceData.concepts.forEach((c: ConceptNode) => addNode({ id: c.id, label: c.label, type: 'concept', data: c }));

            const filteredEvidence = filterDocs(evidenceData.docs);
            filteredEvidence.forEach(d => {
                addNode({ id: d.id, label: d.name, type: 'doc', data: d });
            });

            // Evidence mode specific edge building (only link *these* docs/concepts)
            buildMeaningfulEdges(filteredEvidence);
            // Optionally prune concept nodes that didn't get connected?
            // For now, keep them if backend returned them.
        }
        else if (effectiveMode === 'local' && activeCenterId) {
            // LOCAL MODE: Center + 1-hop
            if (activeCenterId.startsWith('vpath-')) {
                const pathString = activeCenterId.replace('vpath-', '');
                addNode({ id: activeCenterId, label: pathString.split('/').pop() || pathString, type: 'virtualPath' });
                const pathDocs = filterDocs(docs.filter(d => d.folderPath.includes(pathString))).slice(0, 50);
                pathDocs.forEach(d => addNode({ id: d.id, label: d.name, type: 'doc', data: d }));

                // Link Path -> Docs
                pathDocs.forEach(d => links.push({ source: activeCenterId, target: d.id, type: 'contains' }));

                // Add meaningful edges among them
                buildMeaningfulEdges(pathDocs);

            } else {
                // Entity Local View
                // ... (Existing logic refined to use buildMeaningfulEdges logic?)
                // For simplicity, reusing manual logic or switch to subset-based approach
                const centerDoc = docs.find(d => d.id === activeCenterId);
                const centerConcept = concepts.find(c => c.id === activeCenterId);

                let localDocs: DocRecord[] = [];

                if (centerConcept) {
                    addNode({ id: centerConcept.id, label: centerConcept.label, type: 'concept', data: centerConcept });
                    localDocs = filterDocs(docs.filter(d => d.conceptIds.includes(centerConcept.id) || d.tags.includes(centerConcept.label))).slice(0, 40);
                } else if (centerDoc) {
                    addNode({ id: centerDoc.id, label: centerDoc.name, type: 'doc', data: centerDoc });
                    // Bring in related docs?
                    // 1. Docs sharing concepts
                    const sharedConcepts = centerDoc.conceptIds;
                    localDocs = filterDocs(docs.filter(d => d.id !== centerDoc.id && d.conceptIds.some(cid => sharedConcepts.includes(cid)))).slice(0, 20);
                    // Add centerDoc back to include in processing
                    localDocs.push(centerDoc);
                }

                localDocs.forEach(d => addNode({ id: d.id, label: d.name, type: 'doc', data: d }));

                // Add concepts for these docs
                const relevantConceptIds = new Set<string>();
                if (centerConcept) relevantConceptIds.add(centerConcept.id);
                localDocs.forEach(d => d.conceptIds.forEach(cid => relevantConceptIds.add(cid)));

                relevantConceptIds.forEach(cid => {
                    const c = concepts.find(x => x.id === cid);
                    if (c) addNode({ id: c.id, label: c.label, type: 'concept', data: c });
                });

                buildMeaningfulEdges(localDocs);
            }
        }
        else {
            // DEFAULT MODE (Global)
            // 1. Top Concepts Limit (e.g., Top 50 by frequency?)
            // For now, just slice concepts
            const sortedConcepts = [...concepts].sort((a, b) => b.createdAt - a.createdAt).slice(0, 50);
            sortedConcepts.forEach(c => addNode({ id: c.id, label: c.label, type: 'concept', data: c }));

            // 2. Sampled Docs (Recent 50)
            const recentDocs = filterDocs(docs).sort((a, b) => b.updatedAt - a.updatedAt).slice(0, 50);
            recentDocs.forEach(d => addNode({ id: d.id, label: d.name, type: 'doc', data: d }));

            // 3. Connect them
            buildMeaningfulEdges(recentDocs);
        }

        // Final clean up of links to ensure both ends exist
        const finalLinks = links.filter(l => {
            const sId = typeof l.source === 'string' ? l.source : l.source.id;
            const tId = typeof l.target === 'string' ? l.target : l.target.id;
            return addedNodeIds.has(sId) && addedNodeIds.has(tId);
        });

        return { rawNodes: nodes, rawLinks: finalLinks };
    ...
    // 4. Camera Focus on Selection
    useEffect(() => {
        if (!selectedDoc || !svgRef.current || !dimensions.width) return;

        // Find the node
        const node = groupedNodes.find(n => n.id === selectedDoc.id);

        // Defensive check: If node not found (e.g. filtered out), stop here to prevent crash
        if (!node || typeof node.x !== 'number' || typeof node.y !== 'number') return;

        const svg = d3.select(svgRef.current);
        const { width, height } = dimensions;

        // Calculate abstract transform
        // We want node at center (width/2, height/2)
        // Current transform T applies: T(x) = k*x + tx
        // We want T(node.x) = width/2
        // So k*node.x + tx = width/2  => tx = width/2 - k*node.x

        // Use current scale if possible, or default to 1.2
        const scale = 1.2;

        // Fix: Ensure x, y are numbers (sometimes D3 strictness causes issues with undefined)
        const targetX = node.x || 0;
        const targetY = node.y || 0;

        const tx = width / 2 - scale * targetX;
        const ty = height / 2 - scale * targetY;

        const zoomIdentity = d3.zoomIdentity.translate(tx, ty).scale(scale);

        svg.transition()
            .duration(750)
            .call(d3.zoom<SVGSVGElement, unknown>().transform, zoomIdentity);

    }, [selectedDoc, groupedNodes, dimensions]);
    ...
    useEffect(() => {
        ...
        node.append("circle")
            ...
            .attr("fill", "#0f172a")
            .attr("stroke", d => {
                if (d.type === 'doc' && selectedDoc && d.id === selectedDoc.id) return '#a5b4fc'; // Indigo-300 Highlight
            ...
            .attr("stroke-width", d => {
                if (d.type === 'doc' && selectedDoc && d.id === selectedDoc.id) return 5; // Bold Highlight
            ...
            {stats.nodes === 0 && (
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                    <div className="text-center space-y-4 opacity-50">
                        ...
                        <div>
                            <h3 className="text-slate-400 font-bold text-sm">No Graph Data</h3>
                            <p className="text-slate-600 text-xs mt-1">Waiting for data or no matches found.</p>
                        </div>
                    </div>
                </div>
            )}
