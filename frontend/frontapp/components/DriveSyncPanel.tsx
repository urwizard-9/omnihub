...
const DriveSyncPanel = () => {
    ...
    const handleEnableWatch = async () => {
        if (!token) return;
        try {
            setLoading(true); // Re-use global loading or add local
            await BackendAPI.registerWatch(token);
            alert("✅ Real-time Auto-Sync Enabled!\n(Webhook registered for your account)");
        } catch (e: any) {
            alert("Failed to enable sync: " + e.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="flex flex-1 h-full overflow-hidden bg-[#09090b] relative">
            ...
            {/* Right: Action Panel (Split Mode) */}
            <div className="w-1/2 bg-[#050508] relative overflow-hidden flex">
                {/* Background */}
                ...
                {/* Grid Split */}
                <div className="h-full w-full flex divide-x divide-white/5 z-10">

                    {/* Column 1: Control (Actions + Monitored List) */}
                    <div className="flex-1 p-6 flex flex-col gap-6 overflow-hidden">

                        {/* Top: Action Card */}
                        <div className="bg-[#1E1F2E] border border-white/5 p-6 rounded-2xl shadow-xl space-y-4 shrink-0">
                            <div className="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-500 rounded-xl flex items-center justify-center shadow-lg shadow-indigo-500/20 mx-auto transform -translate-y-2">
                            ...
                            <div className="text-center space-y-0.5">
                                ...
                                <div className="flex flex-col items-center gap-1">
                                    <p className="text-[10px] font-mono text-slate-500 bg-black/30 py-0.5 px-2 rounded inline-block max-w-[150px] truncate">
                                        {(selectedItem ? selectedItem.id : currentFolderId)}
                                    </p>

                                    {/* Status Badge */}
                                    {itemSyncStatus?.exists && (
                                        <div className="flex items-center gap-1 text-[10px] text-indigo-300 bg-indigo-500/10 px-2 py-0.5 rounded-full border border-indigo-500/20">
                                            <CheckCircle size={10} />
                                            <span>Already Synced</span>
                                            {itemSyncStatus.last_synced && (
                                                <span className="text-indigo-400 opacity-70">
                                                    ({new Date(itemSyncStatus.last_synced).toLocaleDateString()})
                                                </span>
                                            )}
                                        </div>
                                    )}
                            ...
                            {isFileSelected ? (
                                ...
                                >
                                    ...
                                    {syncing ? "Ingesting..." : (itemSyncStatus?.exists ? "Update File" : "Ingest File")}
                                </button>
                            ) : (
                                // Folder Actions
                                <>
                                    {isMonitored ? (
                                        ...
                                        >
                                            {syncing ? <RefreshCw size={14} className="animate-spin" /> : <CloudUpload size={16} />}
                                            {syncing ? "Starting..." : (itemSyncStatus?.exists ? "Re-Scan Folder" : "Start Sync")}
                                        </button>
                                    )}
                                </>
                            )}
                        ...
                        <div className="flex items-center justify-between bg-[#1E1F2E] p-4 rounded-xl border border-white/5">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-[10px] text-slate-500">Real-time updates via Webhook</p>
                                </div>
                            </div>
                            <button
                                onClick={handleEnableWatch}
                                className="px-3 py-1.5 bg-indigo-600/20 hover:bg-indigo-600/30 text-indigo-300 text-xs font-bold rounded-lg border border-indigo-500/20 transition-all hover:text-white"
                            >
                                Enable Now
                            </button>
                        </div>

                        {/* Bottom: Active Syncs List (Monitored Folders) */}
                        <div className="flex-1 overflow-hidden">
                            <MonitoredFolderList refreshTrigger={refreshKey} />
                        </div>
                    </div>

                    {/* Column 2: Activity (Queue) */}
                    <div className="flex-1 p-6 flex flex-col overflow-hidden bg-black/10">
                        <h3 className="font-bold text-slate-400 text-xs uppercase mb-3 px-1">Upload Activity</h3>
                        <div className="flex-1 overflow-visible relative">
                            {/* Reusing existing Queue, slightly adjusted height/container */}
