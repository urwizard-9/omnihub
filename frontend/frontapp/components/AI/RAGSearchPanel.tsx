...
const RAGSearchPanel: React.FC<Props> = ({ onCitationClick }) => {
    ...
    const getDocMetadata = (docId: string) => {
        const found = docs.find(d => d.id === docId);
        if (found) {
            return {
                owner: found.owner.split('@')[0],
                isSSOT: found.ssotRating === 'gold' || found.ssotRating === 'silver',
                rating: found.ssotRating
            };
        }
        return { owner: 'System', isSSOT: false, rating: null };
    };
                                    ...
                                    <div className="mt-2 bg-[#0B0C15]/50 rounded-xl border border-white/5 overflow-hidden">
                                        ...
                                        <div className="divide-y divide-white/5">
                                            {ragResponse.citations.map((cit) => {
                                                const meta = getDocMetadata(cit.doc_id);
                                                return (
                                                    <button
                                                        key={cit.idx}
                                                    ...
                                                    >
                                                        <div className="shrink-0 text-slate-500 group-hover:text-indigo-400 mt-0.5">
                                                            {meta.isSSOT ? <Star size={14} className="fill-amber-400 text-amber-400" /> : <FileText size={14} />}
                                                        </div>
                                                        <div className="min-w-0 flex-1">
                                                            ...
                                                            <div className="flex gap-2 items-center">
                                                                ...
                                                                {meta.isSSOT && <span className="text-[9px] text-amber-500 font-bold ml-1">SSOT</span>}
                                                            </div>
