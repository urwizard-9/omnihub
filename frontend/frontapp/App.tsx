...
import { Network, Settings, UserCircle, ShieldCheck, Loader2, AlertCircle, X, LogOut, RefreshCw } from 'lucide-react';
...
const MainLayout: React.FC = () => {
  const {
    ...
    isSyncLocked, enableAutoSync
  } = useOmniHub();

  const [showAdminPanel, setShowAdminPanel] = React.useState(false);

  // Redirect non-admin users from Security tab
  React.useEffect(() => {
    if (activeTab === TABS.SECURITY && !isAdmin) {
      setActiveTab(TABS.OMNIHUB);
    }
              ...
              <div className="bg-[#1E1F2E] border border-white/10 rounded-xl shadow-2xl p-2">
                ...
                {isAuthenticated && (
                  <>
                    <button
                      onClick={() => {
                        if (confirm("Enable Auto-Sync Webhook?")) enableAutoSync();
                      }}
                      className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-indigo-400 text-sm hover:bg-indigo-500/10 font-medium transition-colors"
                    >
                      <RefreshCw size={14} />
                      Enable Auto-Sync
                    </button>
